		...
		if (!file.isHidden() || (flags & LS_A)) {
			...
			if (flags & !LS_DTREE) {
				...
				if (!file.isDir()) {
					...
					file.getName(tempfilename,15);
					
					//Serial.println(tempfilename+String(strlen(tempfilename)));
					for (int i=0;i<strlen(tempfilename);i++) pr->write(tempfilename[i]);
					//file.printName(pr);
					pr->write('\r');
					pr->write('\n');
				}
			...
			if ((flags & LS_R) && file.isDir()) {
				...
				file.lsl(pr, flags, indent + 2,tempfilename);
			}
		}
		file.close();
	}
...
//------------------------------------------------------------------------------
void FatFile::lsl(print_t* pr, uint8_t flags, uint8_t indent, char dirname[32]) {
	FatFile file;
	
	char localdirname[32]={};
	char appendedlocaldirname[32]={};
	sprintf(localdirname,"%s/",dirname);
	rewind();
	...
	while (file.openNext(this, O_READ)) {
		...
		if (!file.isHidden() || (flags & LS_A)) {
			...
			if (flags & LS_DTREE) {
				//Serial.print(localdirname);
				if (!file.isDir()){
					pr->print(localdirname);
					file.printName(pr);
					pr->write('\r');
					pr->write('\n');
				}
			...
			if ((flags & LS_R) && file.isDir()) {
				char tempfilename[13];
				file.getName(tempfilename,13);
				sprintf(appendedlocaldirname,"%s%s",localdirname,tempfilename);
				file.lsl(pr, flags, indent + 2,appendedlocaldirname);
				sprintf(localdirname,"%s",localdirname);
			}
		}
		file.close();
	}
