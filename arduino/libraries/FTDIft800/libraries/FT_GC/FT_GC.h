...
#define		FT_DISPLAY_WVGA_800x480 	2UL
...
class FT_GC : public FT_Trans
{
	...
	void DisplayPWM(uint8_t duty);
	void DisplayPWM0(void);
	void DisplayPWM1(void);
	void AudioOn(void);
	void AudioOff(void);
	...
	//FT_GEStatus WriteCmdfromflash(const prog_uchar *Src,uint32_t NBytes);
	...
	FT_GEStatus TransferCmdfromflash(const prog_uchar *Src,uint32_t NBytes);
...
template<class FT_Trans>
/* Apis to enable/disable backlight */
void FT_GC<FT_Trans>::DisplayPWM(uint8_t duty)
{
	/* switch on the display, 1 means enable and 0 means disable */
	FT_Trans::Write(REG_PWM_DUTY,duty);
	
}
template<class FT_Trans>
/* Apis to enable/disable backlight */
void FT_GC<FT_Trans>::DisplayPWM0(void)
{
	/* switch on the display, 1 means enable and 0 means disable */
	FT_Trans::Write(REG_PWM_DUTY,0);
	
}
template<class FT_Trans>
/* Apis to enable/disable backlight */
void FT_GC<FT_Trans>::DisplayPWM1(void)
{
	/* switch on the display, 1 means enable and 0 means disable */
	FT_Trans::Write(REG_PWM_DUTY,0x80);
...
void FT_GC<FT_Trans>::ActiveInternalClock(void)
{
	uint8_t FT_DLCODE_BOOTUP[12] =
	{
	  ...
	  0,0,0,0x02,		//GPU instruction CLEAR_COLOR_RGB - black color
	  
	  0x07,0,0,0x26,	//GPU instruction CLEAR    FT810
...
void FT_GC<FT_Trans>::DisplayConfigExternalClock(uint8_t ResType)
{
	...
	else if(ResType == FT_DISPLAY_WVGA_800x480)
	{
		FT_Trans::Write16(REG_VSYNC0, 		0 );
		FT_Trans::Write16(REG_VSYNC1, 		10 );
		FT_Trans::Write16(REG_VOFFSET, 		12);
		FT_Trans::Write16(REG_VCYCLE, 		500 );
		FT_Trans::Write16(REG_HSYNC0, 		0 );
		FT_Trans::Write16(REG_HSYNC1, 		41 );
		FT_Trans::Write16(REG_HOFFSET, 		43);
		FT_Trans::Write16(REG_HCYCLE, 		900 );
		FT_Trans::Write16(REG_HSIZE,		800 	);
		FT_Trans::Write16(REG_VSIZE, 		480 	);
		FT_Trans::Write16(REG_PCLK_POL, 	1);
		FT_Trans::Write16(REG_SWIZZLE, 		0);
		/* after configuring display parameters, configure pclk */
		FT_Trans::Write16(REG_PCLK,			3	);
		
		/*
		FT_Trans::Write16(REG_VSYNC0, 		FT_DISPLAY_VSYNC0_WVGA );
		FT_Trans::Write16(REG_VSYNC1, 		FT_DISPLAY_VSYNC1_WVGA );
		FT_Trans::Write16(REG_VOFFSET, 		FT_DISPLAY_VOFFSET_WVGA);
		FT_Trans::Write16(REG_VCYCLE, 		FT_DISPLAY_VCYCLE_WVGA );
		FT_Trans::Write16(REG_HSYNC0, 		FT_DISPLAY_HSYNC0_WVGA );
		FT_Trans::Write16(REG_HSYNC1, 		FT_DISPLAY_HSYNC1_WVGA );
		FT_Trans::Write16(REG_HOFFSET, 		FT_DISPLAY_HOFFSET_WVGA);
		FT_Trans::Write16(REG_HCYCLE, 		FT_DISPLAY_HCYCLE_WVGA );
		FT_Trans::Write16(REG_HSIZE,		FT_DISPLAY_HSIZE_WVGA 	);
		FT_Trans::Write16(REG_VSIZE, 		FT_DISPLAY_VSIZE_WVGA 	);
		FT_Trans::Write16(REG_PCLK_POL, 	FT_DISPLAY_PCLKPOL_WVGA);
		FT_Trans::Write16(REG_SWIZZLE, 		FT_DISPLAY_SWIZZLE_WVGA);
		// after configuring display parameters, configure pclk 
		FT_Trans::Write16(REG_PCLK,			FT_DISPLAY_PCLK_WVGA	);*/
	}
	else
	{
...
template<class FT_Trans>
FT_GEStatus FT_GC<FT_Trans>::TransferCmdfromflash( const prog_uchar *Src,uint32_t NBytes)
{
	uint32_t i,Count;
	//align the NBytes to multiple of 4
	NBytes = (NBytes + 3)&(~3);
...
FT_GEStatus FT_GC<FT_Trans>::WriteCmdfromflash( const prog_uchar *Src,uint32_t NBytes)
{
	FT_GEStatus Status;
	if(0 == TrnsFlag)
	{
